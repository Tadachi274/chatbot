...
class LengthTab(StyleSampleAudioMixin, tk.Frame):
    def __init__(self, parent, profile_store, tts_client, status_var, on_saved=None):
        ...
        self.selected_length = tk.StringVar(
            value=length.get("id", "middle")
        )

        self.other_example1 = tk.StringVar(
            value=length.get("other_example1", "")
        )

        self.other_example2 = tk.StringVar(
            value=length.get("other_example2", "")
        )

        self.length_test_text = tk.StringVar(
    ...
    def build_ui(self):
        ...
        ui.label(
            page,
            text="敬語・親しみ・語彙は変えずに、発話の長さだけを比較します。",
            font="body",
            bg="main_card",
            fg="sub_text",
        ).pack(
    ...
    def build_choice_card(self, parent, opt):
        ...
        if opt["id"] == "other":
            ...
            ui.label(
                ...
                bg="card",
                fg="text",
                wraplength=ui.LAYOUT["card_text_wrap"]+30,
                justify="left",
                anchor="w",
            ).pack(
                fill="x",
    ...
    def build_bottom_area(self, parent):
        ...
        ui.entry(
            input_area,
            textvariable=self.length_test_text,
            font="input",
        ).pack(fill="x", ipady=8)
    ...
    def find_option(self, option_id):
        for opt in LENGTH_BASE_OPTIONS:
            if opt["id"] == option_id:
                return opt

        return LENGTH_BASE_OPTIONS[1]

    def get_current_data(self):
        opt = self.find_option(self.selected_length.get())
    ...
    def save_selection_only(self):
        ...
        self.profile_store.set(
            "length_test_text",
            self.length_test_text.get(),
            auto_save=True,
        )

        self.status_var.set("長さ設定を保存しました")
    ...
    def speak_sample(self):
        ...
        self.status_var.set(f"長さサンプルを再生しました: {text}")
