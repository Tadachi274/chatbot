...
class IntimacyTab(StyleSampleAudioMixin, tk.Frame):
    def __init__(self, parent, profile_store, tts_client, status_var, on_saved=None):
        ...
        self.choice_section = None

        intimacy = self.profile_store.get_nested("intimacy", {})

        self.selected_intimacy = tk.StringVar(
            value=intimacy.get("id", "middle")
        )

        self.other_example1 = tk.StringVar(
            value=intimacy.get("other_example1", "")
        )

        self.other_example2 = tk.StringVar(
            value=intimacy.get("other_example2", "")
        )

        self.intimacy_test_text = tk.StringVar(
    ...
    def get_person_key(self):
        ...
        if speaker.startswith("kenta"):
            return "kenta"
        if speaker.startswith("nozomi"):
            return "nozomi"
    ...
    def get_politeness_id(self):
        ...
        politeness_id = politeness.get("id", "formal")

        if politeness_id == "other":
            return "formal"
    ...
    def get_options(self):
        ...
        person_options = INTIMACY_OPTIONS_BY_PERSON_AND_POLITENESS.get(
            person_key,
            INTIMACY_OPTIONS_BY_PERSON_AND_POLITENESS["nozomi"],
        )

        return person_options.get(
            politeness_id,
            person_options["formal"],
        )
    ...
    def build_ui(self):
        ...
        ui.label(
            page,
            text="選択済みの話者と敬語に合わせて、親しみの出し方を比較します。",
            font="body",
            bg="main_card",
            fg="sub_text",
    ...
    def build_choice_card(self, parent, opt):
        ui.radio(
            parent,
            text=opt["label"],
            variable=self.selected_intimacy,
            value=opt["id"],
    ...
    def build_bottom_area(self, parent):
        ...
        ui.entry(
            input_area,
            textvariable=self.intimacy_test_text,
            font="input",
        ).pack(fill="x", ipady=8)
    ...
    def get_current_data(self):
        opt = self.find_option(self.selected_intimacy.get())
    ...
    def save_selection_only(self):
        ...
        self.profile_store.set(
            "intimacy_test_text",
            self.intimacy_test_text.get(),
            auto_save=True,
        )

        self.status_var.set("親しみ設定を保存しました")
    ...
    def refresh_from_profile(self):
        ...
        # 条件が変わったら、選択値を中に戻す
        self.selected_intimacy.set("middle")
        ...
        self.status_var.set("話者・敬語に合わせて親しみ候補を更新しました")
    ...
    def speak_sample(self):
        text = self.intimacy_test_text.get().strip()
        ...
        self.status_var.set(f"親しみサンプルを再生しました: {text}")
