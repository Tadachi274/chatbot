...
class FillerController:
    ...
    def _filler_choice(self, canthear:bool=False, listen:bool=False):
        ...
        if canthear:
            folder_path = dir_path / "canthear" / f"{level}"
        else:
            folder_path = dir_path / f"{level}"
        wav_files = list(folder_path.glob("*.wav"))
        wav_path = random.choice(wav_files)

        if listen:
            wav_path = next(folder_path.glob("はい*"), None)
            if wav_path is None:
                wav_path = next(folder_path.glob("んん*"), None)

        if self.noding_first and not canthear:
            dir_path = Path(__file__).resolve().parent.parent / "filler" / f"{p['type']}" / "normal"
    ...
    def create_lookaway(self,payload,level):
        print(f"   [create_lookaway] {payload} {level}")
    ...
    def _create_listen_nod(self):
        ...
        if v['level'] == 1:
            wav_path = self._filler_choice(listen=True )
    ...
    def face_cant_hear(self):
        ...
        if v['level'] == 1:
            ...
            self._player.play_later(wave_path)
    ...
    def tick(self):
        ...
        if act >= 1:
        #### actが0→1以上になった(話はじめた)瞬間を検出して時刻を記録
            if self._prev_act < 1:
                self._act_on_t = now
